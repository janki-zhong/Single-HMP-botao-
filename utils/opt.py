...
class Options:
    ...
    def _initial(self):
        ...
        self.parser.add_argument('--num_stage', type=int, default=14, help='size of each model layer')
        ...
        self.parser.add_argument('--epoch', type=int, default=70)
    ...
    def parse(self):
        ...
        if not self.opt.is_eval:
            ...
            # 1. 获取当前日期和时间，格式化为 年-月-日_时-分-秒
            current_time = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')

            # 2. 将时间戳拼接到 log_name 中
            log_name = '{}_{}_in{}_out{}_ks{}_dctn{}'.format(
                current_time,  # 加入当前时间
                script_name,
                self.opt.input_n,
                self.opt.output_n,
