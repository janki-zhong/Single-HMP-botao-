...
def fkl_torch(angles, parent, offset, rotInd, expmapInd, dev='cpu'):
    """
    ...
    p3d = Variable(torch.from_numpy(offset)).float().to(dev).unsqueeze(0).repeat(n, 1, 1)
    angles = angles[:, 3:].contiguous().view(-1, 3)
    R = data_utils.expmap2rotmat_torch(angles, dev).view(n, j_n, 3, 3)
...
def main():
    ...
    # exp1 = Variable(torch.from_numpy(np.vstack((expmap_pred, expmap_gt))).float()).to(device)
