...
def readCSVasFloat(filename):
    ...
    """
    returnArray = []
    # filename = './datasets/h3.6m/dataset/S1/walking_1.txt'
    # filename = filename[1:]
...
def load_data_cmu_3d(path_to_dataset, actions, input_n, output_n, data_std=0, data_mean=0, is_test=False):
    ...
    for action_idx in np.arange(nactions):
        ...
        for examp_index in np.arange(count):
            ...
            exptmps = torch.from_numpy(action_sequence).float().to(opt.dev)
...
def rotmat2euler_torch(R):
    ...
    eul = torch.zeros(n, 3).float().to(opt.dev)
    ...
    if len(idx_spec1) > 0:
        ...
        eul_spec1 = torch.zeros(len(idx_spec1), 3).float().to(opt.dev)
        eul_spec1[:, 2] = 0
        eul_spec1[:, 1] = -np.pi / 2
    ...
    if len(idx_spec2) > 0:
        ...
        eul_spec2 = torch.zeros(len(idx_spec2), 3).float().to(opt.dev)
        eul_spec2[:, 2] = 0
        eul_spec2[:, 1] = np.pi / 2
    ...
    if len(idx_remain) > 0:
        ...
        eul_remain = torch.zeros(len(idx_remain), 3).float().to(opt.dev)
...
def rotmat2quat_torch(R):
    ...
    q = torch.zeros(R.shape[0], 4).float().to(opt.dev)
...
def expmap2rotmat_torch(r, dev='cpu'):
    """
    ...
    R = torch.eye(3, 3).repeat(n, 1, 1).float().to(dev) + torch.mul(
        torch.sin(theta).unsqueeze(1).repeat(1, 9).view(-1, 3, 3), r1) + torch.mul(
        (1 - torch.cos(theta).unsqueeze(1).repeat(1, 9).view(-1, 3, 3)), torch.matmul(r1, r1))
    return R


def expmap2xyz_torch(expmap, dev='cpu'):
    """
    ...
    xyz = forward_kinematics.fkl_torch(expmap, parent, offset, rotInd, expmapInd, dev=dev)
    return xyz
