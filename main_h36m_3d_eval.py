...
def main(opt):
    ...
    ckpt['state_dict'] = cleaned_state_dict
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        # p3d_out_all = net_pred(p3d_src, input_n=in_n, output_n=25, itera=itera, dev=dev)  # long
        # p3d_out_all = p3d_out_all[:, seq_in:].transpose(1, 2).reshape([batch_size, 10 * itera, -1])[:, :out_n]
        # p3d_out_all = p3d_out_all[:, seq_in:].transpose(1, 2).reshape([batch_size, 10 * itera, -1])
        # p3d_out_all, gum_ret, gum_ret_test = net_pred(p3d_src, input_n=in_n, output_n=out_n, itera=itera, dev=dev)
        # p3d_out_all = p3d_out_all[:, seq_in:].transpose(1, 2).reshape([batch_size, 25, -1])[:, :out_n]   # long
