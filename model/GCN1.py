...
class GraphConvolution(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True, node_n=48, is_ga=False, joint=False):
        ...
        self.att_j = Parameter(torch.FloatTensor(40, 40))
    ...
    def reset_parameters(self):
        ...
        self.att_j.data.uniform_(-stdv, stdv)
    ...
    def forward(self, input, y0=None, layer_nums=0, joint=False):
        ...
        if self.joint == False:
            ...
            if layer_nums != 0:
                ...
                support = (1 - alpha) * torch.matmul(self.att_j, input) + alpha * y0
                # output = torch.matmul(support, beta * self.weight + (1 - beta) * self.I.to(self.weight.device))
                output = torch.matmul(support, beta * hamilton + (1 - beta) * self.I.to(self.weight.device))
            else:
                # support = torch.matmul(input, self.weight)
                support = torch.matmul(input, hamilton)
                output = torch.matmul(self.att_j, support)
...
class GC_Block(nn.Module):
    ...
    def forward(self, x, y0=None, y0_s2=None, y0_s3=None, layer_nums=0, joint=False):
        ...
        if self.flag == 1:
            ...
            if joint == False:
                y = self.gc2(y, y0, layer_nums)
                b, n, f = y.shape
                y = self.bn2(y.view(b, -1)).view(b, n, f)
            else:
                y = self.gc2_j(y, y0, layer_nums)
                b, n, f = y.shape
            ...
            if joint == False:
                y = self.gc3(y, y0, layer_nums)
                b, n, f = y.shape
                y = self.bn2(y.view(b, -1)).view(b, n, f)
            else:
                y = self.gc3_j(y, y0, layer_nums)
                b, n, f = y.shape
...
class GCN1(nn.Module):
    def __init__(self, input_feature, hidden_feature, p_dropout, num_stage=1, node_n=48, joint=False):
        ...
        self.conv1d = nn.Conv1d(in_channels=66, out_channels=256, kernel_size=1)
        self.conv2d = nn.Conv1d(in_channels=256, out_channels=66, kernel_size=1)
        self.joint = joint
    ...
    def forward(self, x, joint=False):
        ...
        if joint==False:
            ...
            y0_j = y
            for i in range(self.num_stage):
                y = self.gcbs[i](y, y0_j, y0_s2, y0_s3, 2 * i + 1, joint=True)
            y = self.gc7_j(y)
            y = y.transpose(1, 2)
