...
class GCN(nn.Module):
    ...
    def forward(self, x):
        """
            ...
            x_s2: 中观/肢体部位初始态
            x_s3: 宏观/全局躯干初始态
            y, y_s2, y_s3: 网络对当前骨架状态的“浅层物理抽象”
            y0, y0_s2, y0_s3: 浅层特征“备份”
        """
        x_s2 = self.s1_2_s2(x)
        x_s3 = self.s1_2_s3(x)
        ...
        # y_s2 = interpolate(y_s2.permute(0, 2, 1), scale_factor=2, mode='nearest').permute(0, 2, 1).contiguous()
        # y_s3 = interpolate(y_s3.permute(0, 2, 1), scale_factor=2, mode='nearest').permute(0, 2, 1).contiguous()
        ...
        for i in range(self.num_stage):
            ...
            # y = self.gcbs[i](y, y0, y0_s2, y0_s3, i)
