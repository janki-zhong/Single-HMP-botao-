...
def main(opt):
    ...
    if not opt.is_eval:
        ...
        for epo in range(start_epoch, opt.epoch + 1):
            ...
            print('testing error (last frame): {:.3f}'.format(ret_test[last_frame_key]))
            ...
            #if ret_test[last_frame_key] < err_best:
            #    err_best = ret_test[last_frame_key]
            #    is_best = True
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        p3d_sup = p3d_h36.clone()[:, :, dim_used][:, -out_n:].reshape([-1, out_n, len(dim_used) // 3, 3])
        ...
        if is_train == 0:
            ...
            loss_all = torch.mean(norm_diff * time_weights)
        ...
        if i % 1000 == 0:
            print('{}/{}|bt {:.3f}s|tt{:.0f}s|gn{}'.format(i + 1, len(data_loader), time.time() - bt, time.time() - st,
                                                           grad_norm))
