...
def main(opt):
    ...
    input_data = torch.randn(1, 66, 66).to(dev)
    ...
    # # 判断是否继续训练
    # if Resume:
    #     path_checkpoint = "checkpoint/main_h36m_3d_in50_out25_ks10_dctn20/ckpt_last.pth.tar"  # 断点权重文件的所在路径
    #     checkpoint = torch.load(path_checkpoint)  # 加载保存的断点文件
    #
    #     net_pred.load_state_dict(checkpoint['state_dict'])  # 加载模型可学习参数
    #
    #     optimizer.load_state_dict(checkpoint['optimizer'])  # 加载优化器参数
    #     start_epoch = checkpoint['epoch'] + 1  # 设置继续上次训练的epoch
    #     lr_now = checkpoint['lr']
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        p3d_sup = p3d_h36.clone()[:, :, dim_used][:, -out_n:].reshape(
            [-1, out_n, len(dim_used) // 3, 3])  # 改
        p3d_src = p3d_h36.clone()[:, :, dim_used]
        # p3d_out_all = net_pred(p3d_src, input_n=in_n, output_n=out_n, itera=itera, dev=dev, is_train=0, bs_i=i)
        ...
        p3d_out[:, :, dim_used] = p3d_out_all[:, :, 0]  # 改
        ...
        # p3d_out_all = p3d_out_all.reshape([batch_size, seq_in + out_n, itera, len(dim_used) // 3, 3])    # 改
        ...
        if is_train <= 1:  # if is validation or train simply output the overall mean error
            ...
            mpjpe_p3d_h36_test = torch.sum(torch.mean(torch.norm(p3d_h36[:, in_n:] - p3d_out, dim=3), dim=2), dim=0)
            m_p3d_h36 += mpjpe_p3d_h36_test.cpu().data.numpy()
        # if is_train <= 1 and mpjpe_p3d_h36 < gum_err_best:
        #     file_path = 'gum_ret.pth'
        #     gum_err_best = mpjpe_p3d_h36
        #     torch.save(gum_ret, file_path)
        #     value = gum_err_best.item()
        #     if i >= 3000:
        #         print(i)
        #         print('gum error:', value)
        #         print(gum_ret)
        #     # print(f"Shape befor loading: {gum_ret.shape}")
        #
        # if is_train == 3 and i == 1:
        #     print(gum_ret_test)
